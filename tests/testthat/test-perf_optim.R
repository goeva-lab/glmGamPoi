...
for (fn in c("predict", "residuals")) {
  ...
  for (pred.type in as.list(eval(formals(fn.type)[["type"]]))) {
    test_that(sprintf("%s w/ type=%s (feat|obs).sub arguments works", fn, pred.type), {
      ...
      expect_equal(ref, fn.w(res, feat.sub = seq_len(nrow(Y))), tolerance = 0)
      expect_equal(ref, fn.w(res, obs.sub = seq_len(ncol(Y))), tolerance = 0)
      expect_equal(ref, fn.w(res, feat.sub = rownames(Y)), tolerance = 0)
      expect_equal(ref, fn.w(res, obs.sub = colnames(Y)), tolerance = 0)
      expect_equal(ref, Reduce(rbind, lapply(seq_len(nrow(Y)), function(i) fn.w(res, feat.sub = i))), tolerance = 0)
      expect_equal(ref, Reduce(cbind, lapply(seq_len(ncol(Y)), function(j) fn.w(res, obs.sub = j))), tolerance = 0)
      expect_equal(ref, Reduce(rbind, lapply(rownames(Y), function(i) fn.w(res, feat.sub = i))), tolerance = 0)
      expect_equal(ref, Reduce(cbind, lapply(colnames(Y), function(j) fn.w(res, obs.sub = j))), tolerance = 0)
    })
  }
}
